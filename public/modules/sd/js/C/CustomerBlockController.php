...
class CustomerBlockController extends Controller
{
    ...
    public function CustomerBlockController($id)
    {
        try {
            ...
            FROM debtors_ledgers DL
            INNER JOIN customer_blocks CB ON DL.customer_id = CB.customer_id
            WHERE DL.document_number = 1000 AND (DL.amount - DL.paidamount) > 0 AND CB.customer_block_id = '.$id);
            ...
            WHERE SI.order_date_time >= DATE_SUB(CURRENT_DATE, INTERVAL 3 MONTH) AND CB.customer_block_id = '.$id);
        ...
        //outstanding balance
        $cus_outsanding = DB::select('SELECT SUM(DL.amount - DL.paidamount) as cus_oustanding FROM debtors_ledgers DL INNER JOIN customer_blocks CB ON DL.customer_id = CB.customer_id WHERE (DL.amount - DL.paidamount) > 0 AND CB.customer_block_id = '.$id);
        if($cus_outsanding){
            array_push($data, $cus_outsanding);
        }

        //rep outstanding
        $rep_outstanding = DB::select('SELECT SUM(DL_rep.amount - DL_rep.paidamount) as rep_oustanding FROM debtors_ledgers DL_rep INNER JOIN customer_blocks CB ON DL_rep.employee_id = CB.employee_id WHERE (DL_rep.amount - DL_rep.paidamount) > 0 AND DL_rep.customer_id = CB.customer_id AND CB.customer_block_id = '.$id);
        if($rep_outstanding){
    ...
    public function loadOutstandingDataToTable($id){
        try{
