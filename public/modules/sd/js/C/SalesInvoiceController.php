...
$results->transform(function ($item) {
  ...
  //  $disabled = "disabled";
   // $buttons = '<button class="btn btn-primary btn-sm" id="btnEdit_' . $item->sales_invoice_Id . '" onclick="btnEdit_(' . $item->sales_order_Id . ', \'' . $status . '\')"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>&#160';
    ...
    /* $encodedManualNumber = $this->base64Encode($item->manual_number); */
...
return response()->json([
    ...
    public function getorderItems($id, $date, $cus, $branch, $location)
    {
        try {
            ...
            $query = "SELECT sales_order_items.item_name,sales_order_items.item_id,sales_order_items.quantity,
            ...
            (
               ...
               (SELECT IFNULL(sd_free_offerd_quantity('" . $cus . "',sales_order_items.item_id,sales_order_items.quantity,'" . $date . "'), 0) AS Offerd_quantity) AS free_quantity
