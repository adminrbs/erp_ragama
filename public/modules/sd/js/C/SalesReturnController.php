    ...
    public function addSalesReturn(Request $request, $id)
    {
        try {
            ...
            if ($Sales_invoice->save()) {
                ...
                //updating debtor ledger paid amount
              /*   $S_id = $Sales_invoice->sales_invoice_id;
                if ($S_id != 'undefined') {

                    $Query_exterNal_number = "SELECT sales_invoices.external_number FROM sales_invoices WHERE sales_invoices.sales_invoice_Id = " . $S_id;
                    $result_exr = DB::select($Query_exterNal_number);

                    if ($result_exr) {
                        $externalNumber = $result_exr[0]->external_number;
                        $query = "SELECT IFNULL(debtors_ledgers.amount, 0) - IFNULL(debtors_ledgers.paidamount, 0) AS balance_amount,debtors_ledgers.amount AS invoice_amount,debtors_ledger_id,debtors_ledgers.paidamount AS paidamount 
                    FROM debtors_ledgers 
                    WHERE debtors_ledgers.external_number = '" . $externalNumber . "'";
                        $result_balance_amount = DB::select($query);

                        if ($result_balance_amount) {
                            $actual_balance_amount = $result_balance_amount[0]->balance_amount;
                            $actual_invoice_amount = $result_balance_amount[0]->invoice_amount;
                            $debtor_ledger_id = $result_balance_amount[0]->debtors_ledger_id;
                            $invocie_paid_amount = $result_balance_amount[0]->paidamount;
                            $debtorsLedgerRecords = DebtorsLedger::where('external_number', '=', $Sales_invoice->external_number)->get();

                            if (floatval($actual_balance_amount) == 0) {

                                foreach ($debtorsLedgerRecords as $record) {
                                    $record->paidamount = 0;
                                    $record->update();
                                }
                            } else if (floatval($actual_balance_amount) > 0) {

                                $debtor_ledger_invoice = DebtorsLedger::find($debtor_ledger_id);
                                if ($debtor_ledger_invoice) {
                                    $return_amount = $Sales_invoice->total_amount;
                                    if (floatval($return_amount) < floatval($actual_balance_amount)) {
                                        $total_paid_amount = floatval($invocie_paid_amount) + floatval($return_amount);
                                        $debtor_ledger_invoice->paidamount = $total_paid_amount;
                                        $debtor_ledger_invoice->update();

                                        foreach ($debtorsLedgerRecords as $record) {
                                            $record->paidamount = -floatval($return_amount);
                                            $record->update();
                                        }
                                    } else if (floatval($return_amount) > floatval($actual_balance_amount)) {
                                        $debtor_ledger_invoice->paidamount = $actual_invoice_amount;
                                        $debtor_ledger_invoice->update();
                                        foreach ($debtorsLedgerRecords as $record) {
                                            $record->paidamount = -floatval($actual_balance_amount);
                                            $record->update();
                                        }
                                    } else if (floatval($return_amount) == floatval($actual_balance_amount)) {
                                        $debtor_ledger_invoice->paidamount = $actual_invoice_amount;
    ...
    public function getInvoiceItemsToreturnTable(Request $request, $branchID, $id, $path)
    {
        try {
           ...
           FROM 
               ...
               INNER JOIN sales_invoices SI ON SI.sales_invoice_Id = SII.sales_invoice_Id
           WHERE 
               SII.external_number = (SELECT external_number FROM sales_invoices WHERE sales_invoices.manual_number = $invoice_id)";
    ...
    public function getHeaderDetailsForInvoiceReturn($id, $path)
    {
        try {
            if ($path == "model") {
                ...
                sales_invoices.payment_term_id FROM sales_invoices INNER JOIN customers ON sales_invoices.customer_id = customers.customer_id INNER JOIN sales_invoice_items ON sales_invoices.sales_invoice_Id = sales_invoice_items.sales_invoice_Id  WHERE  sales_invoices.manual_number = '" . $id . "' AND (ABS(sales_invoice_items.quantity) - sales_invoice_items.returned_qty) > 0";
                $result = DB::select($query);
                if ($result) {
                    return response()->json(['success' => 'Data loaded', 'data' => $result]);
                } else {
...
";
    ...
    public function load_setoff_data_($id){
     ...
     public function  load_setoff_data_invoice($id){
        try{
            $cus_id = DB::select('SELECT customer_id FROM sales_invoices SI WHERE SI.manual_number = "' . $id . '"');
            
            $qry = "SELECT 
            dl.debtors_ledger_id, 
            dl.trans_date, 
            IFNULL(dl.manual_number, dl.external_number) AS manual_number, 
