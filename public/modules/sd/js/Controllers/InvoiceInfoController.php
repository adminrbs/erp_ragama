...
class InvoiceInfoController extends Controller
{
    ...
    public function load_invoice_details($number){
        $inv_data_header_qry = 'SELECT SI.external_number, SI.order_date_time, B.branch_name, L.location_name, C.customer_name, E.employee_name,DL.amount,DL.paidamount, (DL.amount - DL.paidamount) as balance, SO.external_number as so_number,SO.order_date_time as s_order_date,DATEDIFF(SI.order_date_time,SO.order_date_time) AS date_gap
    ...
    //items
    $item_data = DB::select('SELECT SI.item_id,SI.item_name,SI.quantity,SI.free_quantity,SI.unit_of_measure,SI.package_size,SI.discount_percentage,SI.price,IT.Item_code FROM sales_invoice_items SI INNER JOIN sales_invoices S ON SI.sales_invoice_Id = S.sales_invoice_Id INNER JOIN items IT ON SI.item_id = IT.item_id WHERE S.manual_number ='.$number);
    
    //sales return
 /*    $return_data = DB::select('SELECT SR.sales_return_Id,SR.manual_number,SR.order_date,SR.total_amount,C.customer_name,E.employee_name FROM sales_returns SR LEFT JOIN sales_invoices SI ON SR.sales_invoice_id = SI.sales_invoice_Id LEFT JOIN customers C ON SR.customer_id = C.customer_id LEFT JOIN employees E ON SR.employee_id = E.employee_id WHERE SI.manual_number = '.$number); */

 /* $return_data = DB::select("
    SELECT SR.sales_return_Id, SR.manual_number, SR.order_date, SR.total_amount, C.customer_name, E.employee_name
    FROM sales_returns SR
    LEFT JOIN sales_invoices SI ON SR.sales_invoice_id = SI.sales_invoice_Id
    ...
    SELECT SR.sales_return_Id, SR.manual_number, SR.order_date, SR.total_amount, C.customer_name, E.employee_name
    FROM sales_returns SR
    LEFT JOIN debtors_ledger_setoffs DLS ON SR.internal_number = DLS.internal_number
    LEFT JOIN sales_invoices SI ON DLS.reference_internal_number = SI.internal_number
    LEFT JOIN sales_return_debtor_setoffs SRD ON SI.internal_number = SRD.internal_number
    LEFT JOIN customers C ON SR.customer_id = C.customer_id
    LEFT JOIN employees E ON SR.employee_id = E.employee_id
    WHERE SI.manual_number = $number
"); */
...
$return_data = DB::select("
    SELECT SR.sales_return_Id, SR.manual_number, SR.order_date, SR.total_amount, C.customer_name, E.employee_name,U.name
    FROM sales_returns SR
    LEFT JOIN sales_invoices SI ON SR.sales_invoice_id = SI.sales_invoice_Id
    LEFT JOIN customers C ON SR.customer_id = C.customer_id
    LEFT JOIN employees E ON SR.employee_id = E.employee_id
    LEFT JOIN users U ON SR.prepaired_by = U.id
    WHERE SI.manual_number = $number
...
");
    ...
    //customer receipt
    $cus = DB::select('SELECT CR.receipt_date, CR.external_number,CR.amount,E.employee_name ,CRH.cheque_number, DATEDIFF(SI.order_date_time,CR.receipt_date) AS Gap,CRS.set_off_amount FROM customer_receipts CR LEFT JOIN employees E ON CR.collector_id = E.employee_id LEFT JOIN customer_receipt_setoff_data CRS ON CR.customer_receipt_id = CRS.customer_receipt_id INNER JOIN sales_invoices SI ON CRS.reference_internal_number = SI.internal_number LEFT JOIN customer_receipt_cheques CRH on CR.customer_receipt_id = CRH.customer_receipt_id WHERE SI.manual_number ='.$number);

    //sfa receipt
    $sfa = DB::select('SELECT
    SFA.receipt_date,
    SFA.external_number,
    SFA.amount,
    SFC.cheque_number,
...
LEFT JOIN
    sfa_receipt_cheques ON SFA.customer_receipt_id = sfa_receipt_cheques.customer_receipt_id
WHERE
    SI.manual_number = '. $number.''
);

    $delivery_plan = DB::select('SELECT DP.delivery_plan_id,DP.external_number,emp_d.nick_name as driver_name,emp_h.nick_name as helper_name,
    V.vehicle_no, U.`name` FROM sales_invoices SI INNER JOIN delivery_plans DP ON SI.delivery_plan_id = DP.delivery_plan_id 
    INNER JOIN employees emp_d ON DP.driver_id = emp_d.employee_id INNER JOIN employees emp_h ON DP.helper_id = emp_h.employee_id 
    INNER JOIN vehicles V ON DP.vehicle_id = V.vehicle_id LEFT JOIN users U ON DP.created_by = U.id WHERE SI.manual_number ='.$number);
     
     $picking_list = DB::select('SELECT DATE(P.created_at) AS created_date,P.delivery_plan_packing_list_id,SI.picking_list_id 
     FROM sales_invoices SI
     INNER JOIN delivery_plan_packing_lists P ON SI.picking_list_id = P.delivery_plan_packing_list_id
     WHERE SI.manual_number = '.$number);

    
   // $dp_id = $delivery_plan[0]->delivery_plan_id;
   
   $inv_ = DB::select('SELECT sales_invoice_Id FROM sales_invoices WHERE manual_number ='.$number);
