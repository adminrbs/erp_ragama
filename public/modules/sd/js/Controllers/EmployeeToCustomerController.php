...
class EmployeeToCustomerController extends Controller
{
        ...
        try {
            ...
            if ($id == 1) {
                if($employee == null){
                    ...
                    (SELECT customer_id  FROM customer_employees  WHERE employee_id=$employee)";
    ...
    public function addEmployeeCustomer(Request $request)
    {
            ...
            try {
                ...
                foreach ($option_array as $user_id) {
                    ...
                    $existingRecord = DB::table('customer_employees')
                        ->where('customer_id', $user_id)
                        ->where('employee_id', $locationid)
                        ->first();
    ...
    public function getEmployeeCustomerDetails()
    {
        ...
        FROM
        customer_employees
            INNER JOIN customers ON customer_employees.customer_id = customers.customer_id
            INNER JOIN employees ON customer_employees.employee_id = employees.employee_id
    ...
    {
        try {
            $query = "SELECT CONCAT(customers.customer_code,'-',customer_name,'-',routes.route_name) AS customer_name, employee_id,customers.customer_id FROM customer_employees
            LEFT JOIN customers ON customer_employees.customer_id = customers.customer_id
