        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                ...
                if (result) {
                    ...
                    newReferanceID('creditor_credit_note', '2300');
                    addCreditNote();
...
function newReferanceID(table, doc_number) {
    referanceID = newID("../newReferenceNumber_creditor_credit_note", table, doc_number);
...
function getEachCreditNote(id){
    $.ajax({
        url: '/sl/getEachCreditNote/'+id,
        type: 'get',
        dataType: 'json',
        async: false,
        success: function (data) {
            var dt = data.data;
            console.log(dt[0].external_number);
            $('#LblexternalNumber').val(dt[0].external_number);
            $('#order_date_time').val(dt[0].trans_date);
            $('#cmbBranch').val(dt[0].branch_id);
            //$('#cmbSalesRep').val(dt[0].external_number);
            $('#txtSupid').val(dt[0].supplier_code);
            $('#lblSupplierName').val(dt[0].supplier_name);
            $('#lblSupplierAddress').val(dt[0].primary_address);
            $('#txtRemarks').val(dt[0].description);
            $('#txtNarration').val(dt[0].narration_for_account);
            $('#txtAmount').val(Math.abs(dt[0].amount).toLocaleString('en-US', { style: 'decimal' }));
