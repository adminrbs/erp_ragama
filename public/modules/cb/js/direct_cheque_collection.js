...
$(document).ready(function () {
    ...
    $('#cmbBranch').on('change', function () {
        ...
        load_direct_cheque_create_to_bundle($('#cmbBranch').val())
    });

    load_direct_cheque_create_to_bundle(0);


    $('#btn_cheque_ho_save').on('click', function () {
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                console.log(result);
                if (result) {
                    newReferanceID('direct_cheque_collections', '950');
                    create_direct_cheque_bundle();
...
//load direct customer receipts for cheque bundle - ho
function load_direct_cheque_create_to_bundle(br_id) {
    if($('#cmbBranch').val() == 0){
    ...
    $.ajax({
        url: '/cb/load_direct_cheque_create_to_bundle/' + br_id,
        type: 'get',
        cache: false,
        timeout: 800000,
        beforeSend: function () { },
        success: function (response) {
            ...
            for (var i = 0; i < dt.length; i++) {
                ...
                var cheque_check_box = '<input class="form-check-input" type="checkbox" id="' + dt[i].customer_receipt_id + '" onchange="update_labels(this)">';
...
function update_labels(event) {
    ...
    var six_cell = $row.find('td:eq(6)');
    var _amount = six_cell.text();
...
//create cash bund;e
function create_direct_cheque_bundle() {
    ...
    $.ajax({
        url: '/cb/create_direct_cheque_bundle',
        method: 'post',
        data: formData,
        processData: false,
        contentType: false,
...
function newReferanceID(table, doc_number) {
    referanceID = newID("../newReferenceNumber_direct_cheque_bundles", table, doc_number);
    console.log(referanceID);
