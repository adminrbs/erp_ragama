...
$(document).ready(function () {
    ...
    load_direct_cash_create_to_bundle(0);
    ...
    $('#btn_cash_ho_save').on('click', function () {
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                console.log(result);
                if (result) {
                    newReferanceID('direct_cash_bundles', '900');
                    create_direct_cash_bundle();
...
function load_direct_cash_create_to_bundle(br_id) {
    if($('#cmbBranch').val() == 0){
    ...
    $.ajax({
        url: '/cb/load_direct_cash_create_to_bundle/' + br_id,
        type: 'get',
        cache: false,
        timeout: 800000,
        beforeSend: function () { },
...
//create cash bund;e
function create_direct_cash_bundle() {
    ...
    $.ajax({
        url: '/cb/create_direct_cash_bundle',
        method: 'post',
        data: formData,
        processData: false,
        contentType: false,
...
function newReferanceID(table, doc_number) {
    referanceID = newID("../newReferenceNumber_direct_cash_bundles", table, doc_number);
    console.log(referanceID);
