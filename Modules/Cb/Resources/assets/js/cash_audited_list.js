...
$(document).ready(function () {
    ...
    $('#cmbBranch').on('change', function () {
        ...
        load_audited_cash_receipts($(this).val(),global_collector_id);
    ...
    $('#cmbEmp').on('change', function () {
        ...
        load_audited_cash_receipts(global_branch_id_,$(this).val());
        $('#sum_label').text('0.00').addClass('h4');
        $('#row_count').text('0');
    });
...
function load_audited_cash_receipts(br_id,collector_id_) {
    if(br_id < 1 || isNaN(br_id)){
        return;
    }
    if(isNaN(collector_id_)){
        return;
    }
   
    console.log(br_id);
    $.ajax({
        url: '/cb/load_audited_cash_receipts/' + br_id +'/'+collector_id_,
        type: 'get',
        cache: false,
        timeout: 800000,
        beforeSend: function () { },
