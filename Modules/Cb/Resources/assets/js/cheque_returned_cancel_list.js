...
$(document).ready(function(){
    ...
    load_dishonoured_cheques_canceled();
...
function approve(id){
    bootbox.confirm({
        ...
        callback: function (result) {
            //console.log('Confirmation result:', result);
            if (result) {
                approve_return_cancelation(id,1); // 1 is approve
            } else {
...
function load_dishonoured_cheques_canceled(){
    
    $.ajax({
            url:'/cb/load_dishonoured_cheques_canceled',
            type:'get',
            cache: false,
            timeout: 800000,
            beforeSend: function () { },
            success: function (response) {
                ...
                for (var i = 0; i < dt.length; i++) {
                   var edit = '<button class="btn btn-success btn-sm" onclick="approve('+dt[i].cheque_returns_id+')" title="Approve"><i class="fa fa-pencil-square-o" aria-hidden="true" ></i></button>';
                   var reject = '<button class="btn btn-danger btn-sm" onclick="reject('+dt[i].cheque_returns_id+')" title="Reject"><i class="fa fa-trash" aria-hidden="true" ></i></button>';
...
function approve_return_cancelation(id,type) {
    $.ajax({
        url: '/cb/approve_return_cancelation/'+id+'/'+type,
        method: 'post',
        data: formData,
        processData: false,
        contentType: false,
...
function reject(id){
        bootbox.confirm({
            ...
            callback: function (result) {
                console.log(result);
                if (result) {
                    approve_return_cancelation(id,2);
                } else {
