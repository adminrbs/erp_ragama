    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i = 0; i < dt.length; i++) {
               ...
               id_array.push(id);
                data.push({
                    "receipt_date":dt[i].receipt_date,
                    "banked_date":dt[i].banking_date,
...
function load_dishonour_reasons(table) {
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(dt, function (key, value) {
                ...
                table.rows().every(function () {
                    // Find the DataTable row index
                    var rowIndex = this.index();

                    // Find the 6th column (index 5) in the current DataTable row and append the option
                    var select = table.cell({ row: rowIndex, column: 5 }).nodes().to$().find('select');
                    select.append("<option value='" + value.cheque_dishonur_reason_id + "'>" + value.cheque_dishonur_reason + "</option>");
                });
            });

            // Trigger change event for the selects
            $('#cheque_dishonur_table select').change();
        },
    });
}
