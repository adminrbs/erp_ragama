    ...
    const _componentDualListbox = function () {
        ...
        const listboxButtons = new DualListbox(listboxButtonsElement, {
            ...
            removeEvent: function (value) {
                var selectedemployee = $('#cmbSalesReps').val();
                

                if (selectedemployee && value) {
                   
                    $.ajax({
                        url: '/sd/selectdeletsupplygroupsalesrep', 
                        type: 'POST', 
                        data: {
                        ...
                        success: function (response) {
                            ...
                            getsupplygrouptosalesrepDteails();
                        },
...
function getFilteredData(id) {
    DualListboxes.clear();
    var selectedselesrep = $('#cmbSalesReps').val();
    var formData = new FormData();
    formData.append('selectedselesrep', selectedselesrep);

    const dualListBox = document.getElementById('cmbFilterData');

    $.ajax({
        url: '/sd/getsupplygrouplistbox/' + id,
        method: 'post',
        ...
        success: function (response) {
            ...
            $.each(data, function (index, item) {
                if (id == 1) {
                    ...
                    DualListboxes.geOptionArray().push({ text: item.supply_group, value: item.supply_group_id  });
...
function getselectsupplygroup(){
    ...
    if(selectuser == "1"){
        ...
        $.ajax({
            url: '/sd/getselectsupplygroup/'+ selectedBranchId,
            method: 'get',
            async: false,
            datatype: 'json',
            success: function (data) {
                ...
                if(dt !=undefined){
                    ...
                    for (var i = 0; i < dt.length; i++) {
                        ...
                        DualListboxes.geOptionArray().push({ text: dt[i].supply_group, value: dt[i].supply_group_id,selected:true});
...
function addsupplygrouptoSalesrep() {
    ...
    $.ajax({
        url: '/sd/addsupplygrouptoSalesrep',
        method: 'post',
...
function getsupplygrouptosalesrepDteails() {
   
    $.ajax({
        type: "GET",
        url: "/sd/getsupplygrouptosalesrepDteails",
        cache: false,
        timeout: 800000,
        beforeSend: function () { },
        success: function (response) {
            ...
            for (var i = 0; i < dt.length; i++) {
                ...
                data.push({
                    "action": '<input class="form-check-input" type="checkbox" name="record[]" value="' + dt[i].employee_id  + '|' + dt[i].supply_group_id + '">',
                    "supplygroup_name": dt[i].supply_group,
            ...
            var table = $('#supplygroup_salesrep').DataTable();
            table.clear();
...
function deletesupplygrouptosalesrep() {
    var selectedRecords = [];
    ...
    $.ajax({
        url: '/sd/deletesupplygrouptosalesrep', // Replace with your Laravel route URL
        type: 'DELETE',
          headers: {
