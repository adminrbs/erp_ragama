...
function getSalesOrderDetails(distributor) {
    $.ajax({
        type: "GET",
        url: "/sd/getLateOrdersDetails/"+ distributor,
        cache: false,
        timeout: 800000,
        beforeSend: function () { },
        success: function (response) {
            ...
            for (var i = 0; i < dt.length; i++) {
                ...
                data.push({
                    ...
                    "customer_name": '<div title="'+dt[i].customer_name+'">'+shortenString(dt[i].customer_name,15)+'</div>',
                    "employee_name": '<div title="'+dt[i].employee_name+'">'+shortenString(dt[i].employee_name,12)+'</div>',
