...
class freeOfferController extends Controller
{
      ...
      public function addFreeOffer(Request $request)
      {
          try {
              ...
              if ($offer->save()) {
                  ...
                  return response()->json(["status" => true, "primaryKey" => $primaryId]);
              } else {
    ...
    public function addOfferData(Request $request, $id)
    {
        try {
            ...
            /*  $query = "SELECT COUNT(*) AS count FROM free_offers INNER JOIN free_offer_datas ON 
            free_offers.offer_id = free_offer_datas.offer_id WHERE free_offer_datas.item_id = '".$itm_id."' AND free_offers.offer_id = '".$offer_id."' AND (free_offers.start_date >= (SELECT free_offers.start_date FROM free_offers WHERE free_offers.offer_id = '".$offer_id."') OR free_offers.start_date <= (SELECT free_offers.start_date FROM free_offers WHERE free_offers.offer_id = '".$offer_id."')
            OR free_offers.end_date >= (SELECT free_offers.end_date FROM free_offers WHERE free_offers.offer_id = '".$offer_id."') OR free_offers.end_date <= (SELECT free_offers.end_date FROM free_offers WHERE free_offers.offer_id = '".$offer_id."'))";
            
            $result = DB::select($query);
            

            if($result[0]->count > 0){
                return response()->json(["status" => false, "message" => "date overlap"]);
            } */
    ...
    public function get_offer_data_apply_to($type)
    {
        try {
            if ($type == 2) {
                $customer = DB::select("SELECT customer_id as id,customer_name as name FROM customers");
        ...
        try {
            ...
            if ($offer_data_qry) {
                foreach ($offer_data_qry as $data) {
                    ...
                    $offer_item_qry = DB::select("SELECT * FROM free_offer_thresholds WHERE free_offer_thresholds.offer_data_id = $offer_data_id");
                    if ($offer_item_qry) {
            ...
            return response()->json(["offer" => $offer_qry, "offer_Data" => $offer_data_qry, "item" => $item_array, "customers" => $offer_customer_qry]);
        } catch (Exception $ex) {
            return $ex;
        }
    }
