...
class SalesRepWiseMonthlySummaryReportController extends Controller
{
  public function salesRepwiseMonthlySummary($search)
  {
    try {
      ...
      if ($search_option[1]->fromdate !== null) {
        $nonNullCount++;
      }
      if ($search_option[2]->todate !== null) {
        $nonNullCount++;
      }

      if ($search_option[0]->selecteBranch !== null) {
        $nonNullCount++;
      }

      if ($search_option[3]->selectSalesrep !== null) {
        $nonNullCount++;
      }
      }
      ...
      (SELECT
        ...
        FROM
          (SELECT
            ...
            SG.supply_group,
            B.branch_name,
            SG.supply_group AS   supplier_name
          FROM
            sales_invoices SI
