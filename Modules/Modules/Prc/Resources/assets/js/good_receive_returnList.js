const DatatableFixedColumns = function () {
    ...
    const _componentDatatableFixedColumns = function () {
        ...
        var table = $('#GR_return_table').DataTable({
            buttons: {            
                dom: {
                    button: {
...
function edit(id, status) {
    ...
    url = "/prc/goodReceiveReturn?id=" + id + "&paramS=" + status + "&action=edit" + "&task=null";
    window.location.href = url;

}

function view(id, status) {
    url = "/prc/goodReceiveReturnview?id=" + id + "&paramS=" + status + "&action=view" + "&task=null";
    window.location.href = url;
}
...
//load data to table
function getGR_return_data() {
    $.ajax({
        type: "GET",
        url: "/prc/getGRRetrundata",
        cache: false,
        timeout: 800000,
        beforeSend: function () { },
        success: function (response) {
            ...
            for (var i = 0; i < dt.length; i++) {
                ...
                var str_primary = dt[i].goods_received_return_Id; 
                ...
                btnEdit = '<button class="btn btn-primary btn-sm" id="btnEdit_' + str_primary + '" onclick="edit(' + str_primary + ',' + str_status + ')" ' + disabled + ' style="display:none;"><i class="fa fa-pencil-square-o" aria-hidden="true" ></i></button>'
                ...
                data.push({
                    ...
                    "action": btnEdit + '&#160<button class="btn btn-success btn-sm" onclick="view(' + str_primary + ',' + str_status + ')"><i class="fa fa-eye" aria-hidden="true"></i></button>&#160'+btnDlt+'&#160<button class="btn btn-secondary btn-sm" onclick="generateGR_returnReport(' + dt[i].goods_received_return_Id + ')"><i class="fa fa-print" aria-hidden="true"></i></button>',
                });
...
function deleteGR_return(id, status) {
    console.log(id);
    $.ajax({
        url: '/prc/deleteGR_return/' + id + '/' + status,
        type: 'delete',
        data: {
