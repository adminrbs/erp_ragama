...
class BranchwiseStockReportController extends Controller
{
    public function branchwiseStockReport($search)
    {
        ...
        try {
            ...
            if ($nonNullCount > 1) {
                $query = 'SELECT items.Item_code,items.item_Name,items.package_unit, 
                IF(branches.branch_id = 3,IFNULL(SUM(item_historys.quantity),0),0) AS branch1,
                IF(branches.branch_id = 4,IFNULL(SUM(item_historys.quantity),0),0) AS branch2,
                IF(branches.branch_id = 5,IFNULL(SUM(item_historys.quantity),0),0) AS branch3,
                (IF(branches.branch_id = 3,IFNULL(SUM(item_historys.quantity),0),0)+
                 IF(branches.branch_id = 4,IFNULL(SUM(item_historys.quantity),0),0)+
				 IF(branches.branch_id = 5,IFNULL(SUM(item_historys.quantity),0),0)) AS quantity
                           FROM item_historys
                ...
                $reportViwer->addParameter("title", "Branchwise Stock Report " . $date_range);
                ...
                if ($todate != null) {
                    $quryModify .= "item_historys.transaction_date ='" . $todate . "'  ";
                    //$quryModify .= "D.trans_date BETWEEN '" . min($fromdate) . "' AND '" . max($todate) . "'";
                ...
                if ($selecteBranch != null) {
                    if (count($selecteBranch) > 1) {
                        $quryModify .= " item_historys.branch_id IN ('" . implode("', '", $selecteBranch) . "') ";
                    } else {
                        $quryModify .= " item_historys.branch_id ='" . $selecteBranch[0] . "'";
                    }
                }

                if ($selecteLocation != null) {
                    if (count($selecteLocation) > 1) {
                        $quryModify .= " item_historys.location_id IN ('" . implode("', '", $selecteLocation) . "') ";
                    } else {
                        $quryModify .= " item_historys.location_id ='" . $selecteLocation[0] . "'";
                    }
                }

                if ($selectedproduct != null) {
                    if (count($selectedproduct) > 1) {
                        $quryModify .= " item_historys.item_id IN ('" . implode("', '", $selectedproduct) . "') ";
                    } else {
                        $quryModify .= " item_historys.item_id ='" . $selectedproduct[0] . "'";
                    }
                }
                ...
                if ($selectSupplygroup != null) {
                    if (count($selectSupplygroup) > 1) {
                        $quryModify .= " items.category_level_3_id IN ('" . implode("', '", $selectSupplygroup) . "') ";
                    } else {
                        $quryModify .= " items.category_level_3_id ='" . $selectSupplygroup[0] . "'";
                    }
                }
            ...
            return $reportViwer->viewReport('branchwiseStockReport.json');
        } catch (Exception $ex) {
            return $ex;
        }
    }
