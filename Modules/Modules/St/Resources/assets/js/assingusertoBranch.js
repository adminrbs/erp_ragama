...
function getFilteredData(id) {
    ...
    $.ajax({
        url: '/st/getuserDataTOlistbox/' + id,
        method: 'get',
        datatype: 'json',
...
function getselectcustomer(){
    ...
    if(selectuser == "1"){
        ...
        $.ajax({
            url: '/st/getselectuser/'+ selectedBranchId,
            method: 'get',
            async: false,
            datatype: 'json',
            success: function (data) {
                ...
                for (var i = 0; i < DualListboxes.geOptionArray().length; i++) {
                    ...
                    if(dt !=undefined){
                        for (var j = 0; j < dt.length; j++) {
                            if(dt[j].id == DualListboxes.geOptionArray()[i].value){
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i = 0; i < dt.length; i++) {
                ...
                data.push({
                    "action": '<input class="form-check-input" type="checkbox" name="record[]" value="' + dt[i].branch_id  + '|' + dt[i].id + '">',
...
function deleteCustomerLocation() {
    ...
    $.ajax({
        url: '/st/deleteuserBranch', // Replace with your Laravel route URL
        type: 'DELETE',
          headers: {
