...
const DatatableFixedColumns = function () {
    ...
    const _componentDatatableFixedColumns = function () {
        ...
        var table = $('#invoice_inquery_list').DataTable({
           /*  buttons: {            
                dom: {
                    button: {
...
$(document).ready(function(){
    ...
    $('#pending_inv_data_list_model').on('hidden.bs.modal', function () {
            $('#txtStatement').val('');
      });
      

    $('#from_date, #to_date, #cmbAny').on('change', function() {
        load_pending_inqueries();
    });
...
//load invoices
function load_pending_inqueries(){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for(var i = 0; i < dt.length; i++){
                ...
                var showModel_btn = '<button type="button" class="btn btn-success" id="'+dt[i].sales_invoice_tracking_inquery_id+'" title="Info" onclick="showModel(this)"><i class="fa fa-info-circle" aria-hidden="true"></i></button>';
                data.push({
