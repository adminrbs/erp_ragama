...
function get_transaction_allocation_details() {
    $.ajax({
        type: "GET",
        url: "/dl/get_transaction_allocation_details",
        cache: false,
        timeout: 800000,
        beforeSend: function () { },
        success: function (response) {
            ...
            for (var i = 0; i < dt.length; i++) {
                btn_info = '<button class="btn btn-success btn-sm tooltip-target" title="Info" onclick="showInfoModel(' + dt[i].customer_transaction_alocation_id + ')"><i class="fa fa-info-circle" aria-hidden="true"></i></button>';
                data.push({
                    "external_number": dt[i].external_number,
                    "date": dt[i].created_date,
                    "code": dt[i].customer_code,
                    "name": dt[i].customer_name,
