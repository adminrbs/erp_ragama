...
class ValuationControllerOld extends Controller
{
    public function printvaluationReport($search)
    {
        ...
        try {
            ...
            if ($nonNullCount > 1) {
                $query = '  SELECT  I.item_id ,I.item_code, I.item_Name , I.package_unit as package_size , IH.in_hand  ,IH.whole_sale_price,ABS(IH.`value`)  FROM items  I 
                INNER JOIN 
                ( SELECT  item_id  , branch_id,location_id,transaction_date, ABS(SUM(quantity-setoff_quantity)) AS in_hand  ,whole_sale_price, (SUM(quantity-setoff_quantity)*whole_sale_price) AS value 
                 FROM item_history_set_offs WHERE  ((quantity - setoff_quantity) > 0) AND transaction_date <= CURDATE()  GROUP BY item_id ) IH  ON IH.item_id=I.item_id';
                ...
                $reportViwer->addParameter("valuation_tabaledata", $result);
            } else {
                ...
                if ($selectecategory1 != null) {
                    if (count($selectecategory1) > 1) {
                        $quryModify .= " I.category_level_1_id IN ('" . implode("', '", $selectecategory1) . "') ";
                    } else {
                        $quryModify .= " I.category_level_1_id ='" . $selectecategory1[0] . "'";
                    }
                }

                if ($selectecategory2 != null) {
                    if (count($selectecategory2) > 1) {
                        $quryModify .= " I.category_level_2_id IN ('" . implode("', '", $selectecategory2) . "') ";
                    } else {
                        $quryModify .= " I.category_level_2_id ='" . $selectecategory2[0] . "'";
                    }
                }

                if ($selectecategory3 != null) {
                    if (count($selectecategory3) > 1) {
                        $quryModify .= " I.category_level_3_id IN ('" . implode("', '", $selectecategory3) . "') ";
                    } else {
                        $quryModify .= " I.category_level_3_id ='" . $selectecategory3[0] . "'";
                    }
                }
                if ($selectSupplygroup != null) {
                    if (count($selectSupplygroup) > 1) {
                        $quryModify .= " I.supply_group_id  IN ('" . implode("', '", $selectSupplygroup) . "') ";
                    } else {
                        $quryModify .= " I.supply_group_id  ='" . $selectSupplygroup[0] . "'";
                    }
                }
            ...
            $reportViwer->addParameter('nameandcurrontDate', 'Stock Valuation AS AT - ' . $currentDate);


            return $reportViwer->viewReport('valuation.json');
        } catch (Exception $ex) {
            return $ex;
        }
    }
