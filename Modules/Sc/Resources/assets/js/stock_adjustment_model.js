...
var calculationsetWholesalprice = 0;
var wh_price =0;
var rt_price = 0;
var ct_price = 0;
...
$(document).ready(function () {
    ...
    $('#batchTableData').on('click', 'tr', function (e) {
        
        wh_price = $(this).find('td:eq(2)').text();
        rt_price = $(this).find('td:eq(3)').text();
        ct_price = $(this).find('td:eq(1)').text();

        $('#batchTableData tr').removeClass('selected');
        $(this).addClass('selected');
...
function setOff() {
    ...
    for (var i = 0; i < rowObjects.length; i++) {
        ...
        if (item) {
            ...
            var total_qty = Math.abs(parseFloat(qty_));

            // console.log(setoff_quantity);
            ...
            if (setoff_quantity == 0) {
                ...
                calculationsetWholesalprice = setWholesalePrice
            ...
            if (parseFloat(total_qty) != parseFloat(setoff_quantity)) {
                if(parseFloat(total_qty) < 0){
                    ...
                    if(i == global_row_index){
                        
                        /*rowObjects[i][4].val(parseFloat(ct_price.replace(/,/g, '')).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                        rowObjects[i][5].val(parseFloat(wh_price.replace(/,/g, '')).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                        rowObjects[i][6].val(parseFloat(rt_price.replace(/,/g, '')).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));*/
                        rowObjects[i][4].val(parseFloat(ct_price).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                        rowObjects[i][5].val(parseFloat(wh_price).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                        rowObjects[i][6].val(parseFloat(rt_price).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                        break;
                    }
                ...
                rowObjects[i][10].val(setoff_quantity);

                var totalvalue = (Math.abs(parseFloat(qty_)) * parseFloat(setWholesalePrice))

                rowObjects[i][7].val(parseFloat(totalvalue).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
...
function setoffItem(collection, hashmap) {
    ...
    for (var count = 0; count < collection.length; count++) {
        ...
        var quantity = Math.abs((parseFloat(collection[count].quantity)));
...
function createSetoffCollection() {
    ...
    for (var val = 0; val < items.length; val++) {
        ...
        if (items[val][1] != undefined) {
            
                var array = items[val][1].toArray();
                for (var i3 = 0; i3 < array.length; i3++) {
                    var setoffObject = array[i3][1];
                    if (setoffObject.getSetoffQuantity() > 0) {
