...
class GoodsReceiveSummeryReportController extends Controller
{
   public function goods_receive_summery_report($filters){
    try {
       ...
       // dd($branch);
        ...
        if ($nonNullCount > 1) {
           ...
           INNER JOIN supply_groups SG ON S.supply_group_id = SG.supply_group_id";
            ...
            $reportViwer->addParameter("goods_receive_data_summery_table", $result);
        } 
        ...
        $reportViwer->addParameter('Date','From :'.$searchOption[0]->fromDate .' '.'To :'.$searchOption[1]->toDate);

        //dd($length . " " . (strlen($filterLabel)));
      


        return $reportViwer->viewReport('goods_receive_summery_report.json');
    } catch (Exception $ex) {
        return $ex;
    }
   }
