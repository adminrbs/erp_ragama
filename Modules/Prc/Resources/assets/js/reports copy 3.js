...
function showPoHelpReport(result, flag) {
    ...
    for (var supplyGroupId in groupedData) {
        if (groupedData.hasOwnProperty(supplyGroupId)) {
            ...
            // Now you can call a function or perform any operations with the groupDataArray

        }
    }

    processGroupData(all_data_array);

    // Define a function that takes an array of objects as an argument
    function processGroupData(data_array) {
        console.log(data_array);
        /* console.log("Processing data for Supply Group ID:", supplyGroupId);
        console.log("Grouped Data:", groupDataArray); */
        /*  for(var i = 0; i < groupDataArray.length; i++){ */

        /*  $.each(groupDataArray, function(index, element) {
                ...
                content: [

                    {
                        table: {
                            ...
                            headerRows: 1,
                            body: [
                                [{ text: 'KANDANA FOOD AND DRUGS (PVT) LTD', fontSize: 14, bold: true, alignment: 'center', border: [false, false, false, false] }],
        ...
        var Body = [
            {
                table: {
                    widths: ['*', 30, col_width, col_width, col_width, col_width, col_width, col_width, col_width, col_width, col_width, col_width, col_width, col_width, col_width, col_width, col_width, col_width],
                    headerRows: 2,

                    body: showPoHelpReportBody(data_array),
                },
...
function showPoHelpReportBody(result) {
    console.log(result);
    ...
    for (var i = 0; i < result.length; i++) {
        ...
        for (var j = 0; j < result[i].length; j++) {
            name = result[i][j].supply_group;
        }
        body.push([
            { text: name, colSpan: 18, fontSize: 10, bold: true, alignment: 'left', border: [false, false, false, false] }, // Header row
        ]);

        for (var j = 0; j < result[i].length; j++) {


            rd_qty_branch_1 += parseFloat(result[i][j].rd_qty_branch_1);
            qoh_qty_branch_1 += parseFloat(result[i][j].qoh_qty_branch_1);
            rd_qty_branch_2 += parseFloat(result[i][j].rd_qty_branch_2);
            qoh_qty_branch_2 += parseFloat(result[i][j].qoh_qty_branch_2);
            rd_qty_branch_3 += parseFloat(result[i][j].rd_qty_branch_3);
            qoh_qty_branch_3 += parseFloat(result[i][j].qoh_qty_branch_3);
            rd_qty_branch_4 += parseFloat(result[i][j].rd_qty_branch_4);
            qoh_qty_branch_4 += parseFloat(result[i][j].qoh_qty_branch_4);
            rd_qty_branch_5 += parseFloat(result[i][j].rd_qty_branch_5);
            qoh_qty_branch_5 += parseFloat(result[i][j].qoh_qty_branch_5);
            rd_qty_branch_6 += parseFloat(result[i][j].rd_qty_branch_6);
            qoh_qty_branch_6 += parseFloat(result[i][j].qoh_qty_branch_6);
            rd_qty_branch_7 += parseFloat(result[i][j].rd_qty_branch_7);
            qoh_qty_branch_7 += parseFloat(result[i][j].qoh_qty_branch_7);

            var total_rd_qty = (
                parseFloat(result[i][j].rd_qty_branch_1) +
                parseFloat(result[i][j].rd_qty_branch_2) +
                parseFloat(result[i][j].rd_qty_branch_3)
            );
            var total_qoh_qty = (
                parseFloat(result[i][j].qoh_qty_branch_1) +
                parseFloat(result[i][j].qoh_qty_branch_2) +
                parseFloat(result[i][j].qoh_qty_branch_3)
            );


            body.push([
                { text: result[i][j].item_Name, fontSize: font_size, bold: false, alignment: 'left', border: [true, true, true, true] },
                { text: result[i][j].package_unit, fontSize: font_size, bold: false, alignment: 'left', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].rd_qty_branch_1).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].qoh_qty_branch_1).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].rd_qty_branch_2).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].qoh_qty_branch_2).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].rd_qty_branch_3).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].qoh_qty_branch_3).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].rd_qty_branch_4).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].qoh_qty_branch_4).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].rd_qty_branch_5).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].qoh_qty_branch_5).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].rd_qty_branch_6).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].qoh_qty_branch_6).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].rd_qty_branch_7).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
                { text: parseFloat(result[i][j].qoh_qty_branch_7).toFixed(0), fontSize: font_size, bold: false, alignment: 'right', border: [true, true, true, true] },
            ...
            // Add a page break after each inner loop
            if (j === result[i].length - 1) {
                body.push([
                    { text: '', colSpan: 18, fontSize: 10, bold: true, alignment: 'left', border: [false, false, false, false] }, // Invisible row for page break
                ]);
            }
        }
    }
    ...
    total_rd_qty = (
        rd_qty_branch_1 + rd_qty_branch_2 + rd_qty_branch_3 +
        rd_qty_branch_4 + rd_qty_branch_5 + rd_qty_branch_6 +
        rd_qty_branch_7
    );
    total_qoh_qty = (
        qoh_qty_branch_1 + qoh_qty_branch_2 + qoh_qty_branch_3 +
        qoh_qty_branch_4 + qoh_qty_branch_5 + qoh_qty_branch_6 +
        qoh_qty_branch_7
    );
