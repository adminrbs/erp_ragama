...
$(document).ready(function(){
    getPendingapprovalsGRReturn();
});


function Approval(id){
    
        url = "/prc/goodReceiveReturn?id=" + id +"&paramS=Original"+"&action=edit"+"&task=approval";
...
function edit(id, status) {
    ...
    url = "/prc/goodReceiveReturn?id=" + id +"&paramS=Original"+"&action=edit"+"&task=null";
    window.open(url, "_blank");

}

function view(id,status){
    url = "/prc/goodReceiveReturn?id=" + id +"&paramS=Original"+"&action=view"+"&task=null";
    window.open(url, "_blank");
}

//load data to table
function getPendingapprovalsGRReturn(){
    $.ajax({
        url:'/prc/getPendingapprovalsGRReturn',
        type:'get',
        cache: false,
        timeout: 800000,
        beforeSend: function () { },
